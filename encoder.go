...
func addToMap(dest *yaml.Node, fieldName, in interface{}, fieldComent string, style yaml.Style, options *Options) error {
	...
	if options.CommentsFlag.enabled() {
		addComment(key, fieldComent, options.CommentsFlag)
	}
